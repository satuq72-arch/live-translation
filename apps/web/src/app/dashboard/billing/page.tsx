...
function BillingContent() {
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-14">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="mb-1.5 text-2xl font-bold tracking-tight text-prose">Abrechnung</h1>
        <p className="text-sm text-muted">Verwalte deinen Plan und sieh deine Nutzung.</p>
      </div>

      {/* Success banner */}
      {justSubscribed && (
        <div className="motion-safe:animate-fade-up mb-5 flex items-center gap-4 rounded-2xl border border-emerald-800/40 bg-emerald-950/30 px-5 py-4">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 ring-1 ring-emerald-700/40">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-emerald-700">Unbegrenzte Übersetzungen sind jetzt aktiv.</p>
          </div>
        </div>
      )}
      ...
      {/* Plan card */}
      <div className="mb-4 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm">
        <div className="border-b border-white/[0.06] px-6 pt-5 pb-4">
          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-dim">Aktueller Plan</p>
        </div>
      ...
      {/* Stats grid */}
      <div className="mb-10 grid grid-cols-2 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm">
        <div className="border-r border-white/[0.06] px-6 py-5">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-dim">Gesamt übersetzt</p>
        ...
        <div className="px-6 py-5">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-dim">
    ...
    <div className="flex items-start justify-between gap-4">
      <div>
        <div className="mb-1 flex items-center gap-2.5">
          ...
          <span className="rounded-full border border-emerald-800/50 bg-emerald-950/40 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-400">
            aktiv
          </span>
        </div>
        {status.periodEnd && (
      ...
      <button
        onClick={onPortal}
        disabled={working}
        className="cursor-pointer whitespace-nowrap rounded-xl border border-rim px-4 py-2 text-sm font-medium text-violet-300 transition hover:border-indigo-600/70 disabled:cursor-not-allowed disabled:opacity-50"
      >
  ...
  return (
    <div>
      <div className="mb-1 flex items-center gap-2.5">
        ...
        <span className="rounded-full border border-red-800/50 bg-red-950/40 px-2.5 py-0.5 text-[11px] font-semibold text-red-400">
          Limit erreicht
        </span>
      </div>
...
function FreePlan({ freeRemaining, freePct, working, onUpgrade }: {
  ...
  return (
    <div>
      ...
      {/* Progress bar */}
      <div className="mb-5 overflow-hidden rounded-full bg-raised" style={{ height: '6px' }}>
        <div
